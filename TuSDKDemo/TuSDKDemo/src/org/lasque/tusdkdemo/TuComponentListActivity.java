...
public class TuComponentListActivity extends TuFragmentActivity implements TuSdkNavigatorBarDelegate {
    ...
    GestureDetector.SimpleOnGestureListener gestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            // 移动距离
            if (e2.getRawX() - e1.getRawX() < ContextUtils.getScreenSize(TuComponentListActivity.this).width * MAX_SLIDE_DISTANCE)
                return false;

            // 滑动位置
            if (e1.getRawX() > ContextUtils.getScreenSize(TuComponentListActivity.this).width * 0.2)
                return false;

            // 滑动速度
            if (Math.abs(velocityX) < Math.abs(velocityY) || velocityX < MAX_SLIDE_SPEED)
                return false;
